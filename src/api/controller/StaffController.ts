...
// Create API
export const CtreateStaff = async(req:Request,res:Response):Promise<any>=>{
  try {
    const data:createStaffDTO1 = req.body
    const createdata = await staffservice.CreateStaff(data)
    if(createdata === null){
      return res.status(404).json({Error:'Already use this name pls enter diffrent satff name !'})
...
export const BookAppointment = async(req:Request,res:Response):Promise<any>=>{
  ...
  return res.status(400).json({Error:"date is not valid"})
    ...
    return res.status(400).json({Error:"staff not available on sunday .."})
  }else{
   res.status(200).json({data:createdata})
  }
  } catch (error) {
...
export const Fetchbookedstaff = async(req:Request,res:Response):Promise<any>=>{
  const id = parseInt(req.params.id)
  const data = await staffservice.Fetchbookedstaff(id)
  res.send(data)
}

export const updateMystaff = async(req:Request,res:Response):Promise<any>=>{
  try {
  const id = parseInt(req.params.id)  
  ...
  return res.status(404).json({Error:'User booked at this time slot pls try after some time !'})
  }else if(data === "userendtimeerror"){
   return res.status(404).json({Error:'User end time is less than your update endTime pls try again !'})
...
export const findstaffById = async(req:Request,res:Response):Promise<any>=>{
  try {
    const id = parseInt(req.params.id) 
...
export const getByCondition = async(req:Request,res:Response):Promise<any>=>{
 try {
  const {day,startTime,endTime} = req.body
...
export const deletestaff = async(req:Request,res:Response):Promise<any>=>{
  try {
    ...
    if(data === "notdelete"){
      return res.status(404).json({Error:'already booked staff, not delete staff at this time !'})
    }else if(data === "notfound"){
...
export const FindWithDate = async(req:Request,res:Response):Promise<any>=>{
  try {
    const {date} = req.body
...
export const findWithUserTimeAndDate = async(req:Request,res:Response):Promise<any>=>{
  try {
    const {date,ustartTime,uendTime} = req.body
  const data = await staffservice.findWithUserTimeAndDate(date,ustartTime,uendTime)
  if(data === "notDate"){
    return res.status(404).json({Error:'Data not found !'})
   }
   return res.status(200).json({data}) 
...
export const findUserWithStaff = async(req:Request,res:Response):Promise<any>=>{
  try {
...
export const HowManyTimeBook = async(req:Request,res:Response):Promise<any>=>{
  try {
...
export const cancelAppointment = async(req:Request,res:Response):Promise<any>=>{
  try {
    ...
    if(data === "notfound"){
     ...
     return res.status(404).json({Error:'Appointment cannot be canceled as it is less than two hour away !'})
    }else if(data === "alreadycancel"){
      return res.status(404).json({Error:'Already Canceled this appointment !'})
     }else{
      return res.status(200).json({data})
    }
  } catch (error) {
...
export const SearchAvailableTime = async(req:Request,res:Response):Promise<any>=>{
  try {
    ...
    if(data === "notfound"){
      ...
      return res.status(404).json({Error:'All staff is not available ! pls try another day '})
    }else{
      return res.status(200).json({data})
    }
  } catch (error) {
...
export const DeleteExpiredAppointment = async(req:Request,res:Response):Promise<any>=>{
  try {
