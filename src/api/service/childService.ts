import { LoginDTO, RegisterChildDTO, RegisterChildDTO1, RegisterDTO } from "../../dto/childDto";
...
export class ChildService {
  ...
  if(existingUser){
    ...
    const months = Math.floor((getmilisecond % (365.25 * 24 * 60 * 60 * 1000)) / (30.44 * 24 * 60 * 60 * 1000));
  ...
  const existingphon = await UserDetail.findOne({where:{phonNo:phonNo.toString()}})
  ...
  const findphone = await UserDetail.findOne({where:{phonNo:phonNo.toString()}})
  if(!findphone){
    return null 
  }
  ...
  const userdata = await UserDetail.findOne({where:{phonNo:data.phoneNO.toString()}})
  if(!userdata){
    return null
  } 
